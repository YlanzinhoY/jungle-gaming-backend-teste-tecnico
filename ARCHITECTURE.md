...
Os testes unitários exercitam o domínio puro, hash/cursor da aplicação, retry da mensageria, métricas e o grafo Fx. A build tag integration isola a suíte que depende do Compose. Essa suíte usa PostgreSQL, Keycloak e SQS reais e atravessa HTTP, workers, inbox/outbox e três processos; migrations up/down do `golang-migrate` são verificadas em um banco temporário separado para não destruir dados locais. Concorrência financeira é validada pelo estado final e pelo ledger, não por detalhes de implementação em memória.

O detector de corrida é executado em Linux porque o Go para Windows exige CGO e um compilador C. O `Dockerfile.test` instala GCC e o perfil `test` do Compose executa a suíte unitária e de integração com `-race` contra as dependências reais. Os dois testes de recuperação que controlam o ciclo de vida dos containers usam a tag adicional `hostrecovery` e são executados pelo host; os comandos e a justificativa estão em `TESTING.md`. Tracing e testes de carga continuam como diferenciais opcionais do enunciado e não foram implementados.
